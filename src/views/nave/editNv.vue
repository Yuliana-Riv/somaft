  ...
  <div id="EditNv">
    ...
    <div class="tbl_fotos">
      ...
      <div class="bdy_foto" v-for="(item, index) in added.fotos" :key="index">
        <img :src="url + 'nave-img/' + item.image" alt="" v-if="item.tipov == 'imagen'" />
...
export default {
  ...
  computed: {
    added() {
      return this.$store.getters["nave/getAdded"];
    },
    identity() {
    ...
    data() {
      return this.$store.getters["nave/data"];
    },
  },
  methods: {
    ...mapActions("nave", ["setAddedNv"]),
    ...mapActions("nave", ["getAllInfoClb"]),
    ...mapActions("nave", ["getInfoByIdNv"]),
    ...mapActions("colaborador", ["getAllInfoClb"]),

    updateAdded: async function () {
      if (!this.added.id) return setTimeout(() => this.$router.go(), 200);
      const result = await this.getInfoByIdNv({
        id: this.added.id,
        option: "nave",
      });

      if (result.id) {
        this.setAddedNv(result);
      }
    },
    addEtiqueta(value) {
      if (value) {
    ...
    submit: async function (
      ...
      let result = await this.$store.dispatch("nave/editItemNv", {
        option: "nave",
        item: {
          id: this.added.id,
          id_colaborador,
    ...
    updatePortada: async function (id, id_nave) {
      let result = await this.$store.dispatch("nave/editItemNv", {
        option: "nave/portada",
        item: { id, id_nave },
      });
    ...
    deleteFoto: async function (id) {
      ...
      let result = await this.$store.dispatch("nave/deleteItemNv", {
        option: "nave/foto",
        id: id,
      });
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          ...
          for (const image of files) {
            ...
            await this.$store.dispatch("nave/addItemNv", {
              option: "nave/foto",
              item: data,
            });
          }
