  ...
  <div id="EditBod">
      ...
      <form
        @submit.prevent="
          submit(
            ...
            added.act_precio,
            added.leyenda,
            added.superficie_bodega,
            added.superficie_oficina,
            added.estacionamiento
          )
        "
      >
        ...
        <div class="grup-form dflx">
          ...
          <div class="boxform">
            <label for="superficie_bodega" class="superficie_bodega"
              >Superficie bodega</label
            >
            <input
              v-model="added.superficie_bodega"
              type="text"
              id="superficie_bodega"
              name="superficie_bodega"
            />
          </div>
          <div class="boxform">
            <label for="superficie_oficina" class="superficie_oficina"
              >Superficie oficina</label
            >
            <input
              v-model="added.superficie_oficina"
              type="text"
              id="superficie_oficina"
              name="superficie_oficina"
            />
          </div>
        </div>
        ...
        <div class="grup-form dflx">
          <div class="boxform">
            <label for="estacionamiento" class="estacionamiento"
              >Estacionamiento (cajones)
            </label>
            <input
              v-model="added.estacionamiento"
              type="number"
              id="estacionamiento"
    ...
    <div class="tbl_fotos">
      ...
      <div class="bdy_foto" v-for="(item, index) in added.fotos" :key="index">
        <img :src="url + 'bodega-img/' + item.image" alt="" v-if="item.tipov == 'imagen'" />
...
export default {
  ...
  computed: {
    added() {
      return this.$store.getters["bodega/getAdded"];
    },
    colaboradores() {
    ...
    data() {
      return this.$store.getters["bodega/data"];
    },
  },
  methods: {
    ...mapActions("bodega", ["setAddedBod"]),
    ...mapActions("bodega", ["getAllInfoClb"]),
    ...mapActions("bodega", ["getInfoByIdBod"]),
    ...mapActions("colaborador", ["getAllInfoClb"]),

    updateAdded: async function () {
      if (!this.added.id) return setTimeout(() => this.$router.go(), 200);
      const result = await this.getInfoByIdBod({
        id: this.added.id,
        option: "bodega",
      });

      if (result.id) {
        this.setAddedBod(result);
      }
    },
    addEtiqueta(value) {
      if (value) {
    ...
    submit: async function (
      ...
      let result = await this.$store.dispatch("bodega/editItemBod", {
        option: "bodega",
        item: {
          id: this.added.id,
          id_colaborador,
    ...
    updatePortada: async function (id, id_bodega) {
      let result = await this.$store.dispatch("bodega/editItemBod", {
        option: "bodega/portada",
        item: { id, id_bodega },
      });
    ...
    deleteFoto: async function (id) {
      ...
      let result = await this.$store.dispatch("bodega/deleteItemBod", {
        option: "bodega/foto",
        id: id,
      });
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          ...
          for (const image of files) {
            ...
            await this.$store.dispatch("bodega/addItemBod", {
              option: "bodega/foto",
              item: data,
            });
          }
