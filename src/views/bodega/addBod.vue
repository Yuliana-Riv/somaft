...
export default {
  ...
  methods: {
    ...
    submit: async function (
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          let result = await this.$store.dispatch("bodega/addItemBod", {
            option: "bodega",
            item: {
              id_colaborador,
              titulo,
          ...
          if (result.status == "error") {
            ...
            const newadded = await this.getInfoByIdBod({
              id: result.added.id,
              option: "bodega",
            });
            this.files = [];
            this.setAddedBod(newadded);
            this.showModal();
          }
        } else {
          this.status = "error";
    ...
    Add: function () {
      ...
      this.superficie_oficina = "";
      this.superficie_bodega = "";

      this.viewModal = false;
      this.setAddedBod("");
    },
    toEdit: function () {
      this.setHistoryOptionBod("Edit");
      this.viewModal = false;
    },
    back: function () {
      setTimeout(() => this.$router.go(), 200);
