      ...
      >
        ...
        <div class="grup-form dflx">
          ...
          <div class="boxform">
            ...
            <div class="options_btn">
              <p
                @click="tipo_de_suelo = 'habitacional'"
                class="option_btn"
                :class="{ active_option: tipo_de_suelo == 'habitacional' }"
              >
                HABITACIONAL
              </p>
              <p
                @click="tipo_de_suelo = 'mixto'"
                class="option_btn"
                :class="{ active_option: tipo_de_suelo == 'mixto' }"
              >
                MIXTO
              </p>
            </div>
...
export default {
  ...
  methods: {
    ...
    submit: async function (
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          let result = await this.$store.dispatch("terrenos/addItemTrr", {
            option: "terrenos",
            item: {
              id_colaborador,
              titulo,
          ...
          if (result.status == "error") {
            ...
            const newadded = await this.getInfoByIdTrr({
              id: result.added.id,
              option: "terrenos",
            });
            this.files = [];
            this.setAddedTrr(newadded);
            this.showModal();
          }
        } else {
          this.status = "error";
