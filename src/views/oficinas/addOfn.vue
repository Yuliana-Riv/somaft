...
export default {
  ...
  methods: {
    ...
    submit: async function (
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          let result = await this.$store.dispatch("oficinas/addItemOfn", {
            option: "oficinas",
            item: {
              id_colaborador,
              titulo,
          ...
          if (result.status == "error") {
            ...
            const newadded = await this.getInfoByIdOfn({
              id: result.added.id,
              option: "oficinas",
            });
            this.files = [];
            this.setAddedOfn(newadded);
            this.showModal();
          }
        } else {
          this.status = "error";
