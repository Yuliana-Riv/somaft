  ...
  <div id="EditOfn">
    ...
    <div class="tbl_fotos">
      ...
      <div class="bdy_foto" v-for="(item, index) in added.fotos" :key="index">
        <img :src="url + 'oficinas-img/' + item.image" alt="" v-if="item.tipov == 'imagen'" />
...
export default {
  ...
  computed: {
    added() {
      return this.$store.getters["oficinas/getAdded"];
    },
    identity() {
    ...
    data() {
      return this.$store.getters["oficinas/data"];
    },
  },
  methods: {
    ...mapActions("oficinas", ["setAddedOfn"]),
    ...mapActions("oficinas", ["getAllInfoClb"]),
    ...
    updateAdded: async function () {
      ...
      const result = await this.getInfoByIdOfn({
        id: this.added.id,
        option: "oficinas",
      });

      if (result.id) {
        this.setAddedOfn(result);
      }
    },
    addEtiqueta(value) {
      if (value) {
    ...
    submit: async function (
      ...
      let result = await this.$store.dispatch("oficinas/editItemOfn", {
        option: "oficinas",
        item: {
          id: this.added.id,
          id_colaborador,
    ...
    updatePortada: async function (id, id_oficina) {
      let result = await this.$store.dispatch("oficinas/editItemOfn", {
        option: "oficinas/portada",
        item: { id, id_oficina },
      });
    ...
    deleteFoto: async function (id) {
      ...
      let result = await this.$store.dispatch("oficinas/deleteItemOfn", {
        option: "oficinas/foto",
        id: id,
      });
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          ...
          for (const image of files) {
            ...
            await this.$store.dispatch("oficinas/addItemOfn", {
              option: "oficinas/foto",
              item: data,
            });
          }
