...
export default {
  ...
  methods: {
    ...
    submit: async function (
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          let result = await this.$store.dispatch("otros/addItemOtr", {
            option: "otros",
            item: {
              id_colaborador,
              titulo,
          ...
          if (result.status == "error") {
            ...
            const newadded = await this.getInfoByIdOtr({
              id: result.added.id,
              option: "otros",
            });
            this.files = [];
            this.setAddedOtr(newadded);
            this.showModal();
          }
        } else {
          this.status = "error";
