  ...
  <div id="EditCss">
    ...
    <div class="tbl_fotos">
      ...
      <div class="bdy_foto" v-for="(item, index) in added.fotos" :key="index">
        <img :src="url + 'casas-img/' + item.image" alt="" v-if="item.tipov == 'imagen'" />
...
export default {
  ...
  computed: {
    added() {
      return this.$store.getters["casas/getAdded"];
    },
    colaboradores() {
    ...
    data() {
      return this.$store.getters["casas/data"];
    },
  },
  methods: {
    ...mapActions("casas", ["setAddedCss"]),
    ...mapActions("casas", ["getAllInfoClb"]),
    ...
    updateAdded: async function () {
      ...
      const result = await this.getInfoByIdCss({
        id: this.added.id,
        option: "casas",
      });

      if (result.id) {
        this.setAddedCss(result);
      }
    },
    addEtiqueta(value) {
      if (value) {
    ...
    submit: async function (
      ...
      let result = await this.$store.dispatch("casas/editItemCss", {
        option: "casas",
        item: {
          id: this.added.id,
          id_colaborador,
    ...
    updatePortada: async function (id, id_casa) {
      let result = await this.$store.dispatch("casas/editItemCss", {
        option: "casas/portada",
        item: { id, id_casa },
      });
