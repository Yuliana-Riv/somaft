  ...
  <div id="EditLcl">
    ...
    <div class="tbl_fotos">
      ...
      <div class="bdy_foto" v-for="(item, index) in added.fotos" :key="index">
        <img :src="url + 'locales-img/' + item.image" alt="" v-if="item.tipov == 'imagen'" />
...
export default {
  ...
  computed: {
    added() {
      return this.$store.getters["locales/getAdded"];
    },
    identity() {
    ...
    data() {
      return this.$store.getters["locales/data"];
    },
  },
  methods: {
    ...mapActions("locales", ["setAddedLcl"]),
    ...mapActions("locales", ["getAllInfoClb"]),
    ...
    updateAdded: async function () {
      ...
      const result = await this.getInfoByIdLcl({
        id: this.added.id,
        option: "locales",
      });

      if (result.id) {
        this.setAddedLcl(result);
      }
    },
    addEtiqueta(value) {
      if (value) {
    ...
    submit: async function (
      ...
      let result = await this.$store.dispatch("locales/editItemLcl", {
        option: "locales",
        item: {
          id: this.added.id,
          id_colaborador,
    ...
    updatePortada: async function (id, id_local) {
      let result = await this.$store.dispatch("locales/editItemLcl", {
        option: "locales/portada",
        item: { id, id_local },
      });
    ...
    deleteFoto: async function (id) {
      ...
      let result = await this.$store.dispatch("locales/deleteItemLcl", {
        option: "locales/foto",
        id: id,
      });
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          ...
          for (const image of files) {
            ...
            await this.$store.dispatch("locales/addItemLcl", {
              option: "locales/foto",
              item: data,
            });
          }
