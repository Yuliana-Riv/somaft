...
export default {
  ...
  methods: {
    ...
    submit: async function (
      ...
      if (tipov == "imagen") {
        if (files.length > 0) {
          let result = await this.$store.dispatch("locales/addItemLcl", {
            option: "locales",
            item: {
              id_colaborador,
              titulo,
          ...
          if (result.status == "error") {
            ...
            const newadded = await this.getInfoByIdLcl({
              id: result.added.id,
              option: "locales",
            });
            this.files = [];
            this.setAddedLcl(newadded);
            this.showModal();
          }
        } else {
          this.status = "error";
